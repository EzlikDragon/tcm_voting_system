<?php
	...
	$hasSelection = false; // Flag to check if any candidates are selected

	while($row = $query->fetch_assoc()){
		$position = slugify($row['description']);
		$pos_id = $row['id'];
		
		// Check if the position has selected candidates
		if(isset($_POST[$position])){
			$hasSelection = true; // At least one selection made
			if($row['max_vote'] > 1){
	...
	// If no selection was made at all
	if (!$hasSelection) {
		$output['error'] = true;
		$output['message'][] = '<li>You must vote for at least one candidate.</li>';
	}
